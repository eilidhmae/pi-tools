...
function runWorker(o: {
  ...
  return new Promise<RunResult>((resolve) => {
    ...
    // far above tail()'s report size and the end-of-output marker the parse
    // keys on, so neither is affected in normal runs.
    ...
    const settle = (code: number | null, extra = "") => {
      ...
      clearTimeout(timer);
      if (extra) out += `\n${extra}`;
      const { reviewPath, verdict } = parseWorkerOutput(out);
      resolve({ ok: code === 0 || reviewPath !== null, reviewPath, verdict, output: out });
    };
