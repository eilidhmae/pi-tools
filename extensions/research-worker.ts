...
function runWorker(o: {
  ...
  return new Promise<RunResult>((resolve) => {
    ...
    const settle = (code: number | null, extra = "") => {
      ...
      const { reportPath } = parseWorkerOutput(text);
      resolve({ ok: code === 0 || reportPath !== null, reportPath, output: text });
    };
...
export function inDispatchedChild(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.PI_RESEARCH_WORKER_CHILD === "1" || env.PI_ADVERSARY_CHILD === "1" || env.PI_PLANNER_CHILD === "1" || env.PI_CODER_CHILD === "1" || env.PI_CODER_REVIEW_CHILD === "1";
}
...
export default function (pi: any) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Verify every worker claim against reality before acting on it — a worker's output is a claim, not a fact. When a worker reports an artifact path, confirm the file actually exists there (ls/read) BEFORE gating it or advancing; a worker can report a path while keeping the content only in its returned text, never writing the file. If it's missing, re-run that worker — never gate a phantom file (the gate FAILs on a missing target) or pass content onward from your own context as if it landed.",
