...
/**
 * True when running inside a jailed child spawned by either dispatcher (a
 * research worker or an adversary review). Such a child must not auto-dispatch
 * another agent. Checking both markers (not just our own) closes the cross-tool
 * path — an adversary child that somehow had this tool available could otherwise
 * dispatch a worker. (In practice the jailed scripts load only research-mode.ts,
 * so this tool is absent in any child; this is the defense-in-depth backstop.)
 */
export function inDispatchedChild(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.PI_RESEARCH_WORKER_CHILD === "1" || env.PI_ADVERSARY_CHILD === "1";
}
