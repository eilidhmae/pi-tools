#!/usr/bin/env bash
# capture-review.sh -- Thin wrapper that runs tools/ts/capture-review.ts.
#
...
# wrapper, for both global (~/.pi/agent/tools/ts/) and local
# (<repo>/tools/bash/ts/) layouts. Repo-tree development uses ../ts/
# from tools/bash/ since the source lives at tools/ts/.
TS_FILE=""
for candidate in \
  "$SCRIPT_DIR/ts/capture-review.ts" \
  "$SCRIPT_DIR/../ts/capture-review.ts" \
  "$HOME/.pi/agent/tools/ts/capture-review.ts"; do
  if [[ -f "$candidate" ]]; then
    TS_FILE="$candidate"
    break
  fi
