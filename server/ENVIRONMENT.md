...
| `MLX_MTP_DRAFT_DISABLE` | Global kill-switch: force-disables the MTP draft for **every** row, beating any per-row `MLX_MTP_DRAFT_<NAME>` value. The one knob to turn the whole feature off | unset (drafts follow per-row settings) | truthy `1`/`true`/`yes`/`on` (case-insensitive) disables all; anything else is ignored | `server/mlx-server.sh:176-177` (checked first in `mtp_draft_repo_for`) |
| `MLX_MTP_NUM_DRAFT_TOKENS` | `--num-draft-tokens` the MTP draft head proposes per verification round when a row opts in | `2` (was 3; measured best for the agentic role on this M5 Max — mlx_lm's own default is 2) | integer | `server/mlx-server.sh:161`, used at :375 |
