...
and adversary roles are defined by skill prompts and harness flags (the
adversary is held read-only via `--no-tools` or the `--research` jail — see
Pi-Specific Notes, not a `--no-write/--no-edit` pair). The model id determines
...
The pi harness reads the model id from `--model` and routes via `models.json` —
generated by `install.sh` and installed at `~/.pi/agent/models.json` (the repo
carries only `server/models.json.template`). Today's Ollama-only deployments are
unaffected: skip the `local-mlx` provider entirely and the routing degrades to
the existing `qwen3-coder:30b` flow.
...
**Tool enforcement (read-only adversary):** there is no single
`--no-write --no-edit` switch in pi 0.77 — read-only is enforced per invocation
path:

- `adversary-pass.sh` runs the model with `--no-tools` (toolless, single-turn;
  content is inlined). Accepts `HEAD`/`STAGED`/`RANGE:<a>..<b>` diff selectors as
  well as file paths.
- `adversary-jailed.sh` runs a tool-enabled reviewer inside the research-mode
  jail: `--research --tools read,grep,find,ls,bash-safe,write-research`. The jail
  disables `write`/`edit`/raw `bash`; `bash-safe` is an allow-only runner, so the
  reviewer navigates the repo read-only but cannot write outside its workspace.
- Quorum peers (`extensions/lib/quorum-peer.ts`) add `--no-write --no-edit` on
  top of the jailed tool set as a defence-in-depth fallback; those two flags are
  NOT the primary mechanism.

**How to invoke an adversary review (deterministic):**

- From an agent/session: call the `adversary-review` tool, or run
  `/adversary-pass <file> [--quorum]` (the human command). Both spawn the jailed
  tool-enabled reviewer and save under `<workspace>/reviews/` in research mode,
  else `./reviews`. They run **in or out of research mode**. To expose the tool
  to a jailed model, admit it in `--tools`, e.g.
  `read,grep,find,ls,bash-safe,write-research,adversary-review`.
- From the shell directly: `adversary-jailed.sh <path>` (navigable, tool-enabled)
  or `adversary-pass.sh <target>` (toolless inline; takes a file path **or** a
  `HEAD`/`STAGED`/`RANGE:` diff selector). Add `--quorum` for peer reviewers on
  CONCERNS/FAIL.

Disambiguate the similarly-named things. The `adversary-review` **tool** and the
`/adversary-pass` **command** both spawn a *separate, independent* jailed reviewer
via `adversary-jailed.sh` (tool-enabled) — note the command `/adversary-pass` is
distinct from the `adversary-pass.sh` **script**, which is the toolless
(`--no-tools`) inline variant. The `/adversary-review` **prompt** is a different
thing again — an in-session self-review checklist that does **not** spawn a
separate reviewer.
...
- **Models / cache:** `HF_HOME` is operator-set; the pi/mlx code default is
  `$HOME/.cache/huggingface`, but this workstation points it at `~/models` (set
  in `.zshrc`) so everything lives under one root. A model dir must be FLAT
  (top-level `config.json`); an HF cache tree (`blobs/refs/snapshots/`) makes the
  server hang on first request.
